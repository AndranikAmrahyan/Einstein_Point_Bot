...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    help_text = (
        ...
        "💾 Экспорт данных \\(модераторы\\):\n"
        "`/export_data` \\- экспорт данных чата в сообщение\n\n"
        "🔄 Восстановление данных \\(модераторы\\):\n"
        "`/restore_data` \\- восстановление данных из сообщения\n\n"
        "🆘 Помощь:\n"
        "`/help`\n\n"
        f"⚡️ Бот создан специально для чата {chat_mention}\n"
        f"👨💻 Создатель бота: [Эйнштейн]({creator_link})"
    )
...
def main():
    ...
    try:
        ...
        # Убеждаемся, что приложение корректно завершено
