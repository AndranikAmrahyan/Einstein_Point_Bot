...
class Config:
    ...
    CREATOR_ID = 5553779390  # "tg://openmessage?user_id=5553779390"
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    help_text = (
        ...
        "🆘 Помощь:\n"
        "`/help`\n\n"
        f"⚡️ Бот создан специально для чата {escape_markdown(Config.MAIN_ALLOWED_CHAT, version=2)}\n"
        f"👨💻 Создатель бота: [Слоняра](tg://openmessage?user_id={Config.CREATOR_ID})"
    )
        ...
        if chat_id not in Config.ALLOWED_CHAT_IDS:
            await context.bot.send_message(
                ...
                text=(
                    ...
                    f"Бота создал: tg://openmessage?user_id={Config.CREATOR_ID}"
                )
            )
